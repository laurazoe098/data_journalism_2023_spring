...
**Answer**
There are many problems with this data. First, the column names are not representative of the data in the column. Next, The first column listing the county's have a bunch of unnecessary periods. Additionally, the larger number have a commas in them.
...
**Answer**
Maryland is now the first line of data as it should be and all the comas are gone. However, the data still has column names that are still labeled "x1" "x2" etc. Additionally the first column has a bunch of periods that are unnecessary. 
...
**Answer**
Now that we have new column names we need to fix the first column that has a bunch of periods that are not necessary for the data and will make filtering more difficult.
```{r}
