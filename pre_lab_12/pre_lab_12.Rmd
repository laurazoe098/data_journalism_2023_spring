...
**Answer**
This data frame about Cardin's press releases has four columns for date, url, text and title.
```{r}
...
**Answer**
It adds the these words to a list of unique words.
```{r}
...
**Answer**
This creates a data frame of 647 words that are lowercase with no punctuation and that are potentially meaningful.
```{r}
...
**Answer**
The graph shows the top 25 words within the text which are all short and not really meaningful.
...
**Answer**
Yes it is way more interesting than before. Now that all the unimportant words are out we can see that a very prevalent word in the press releases is 'health'.
```{r}
unique_words %>%
...
**Answer**
This creates a top 10 list of words for each year. 'Health' was the top word in 2021, 'project' was the top word in 2022 and 'act' was the top word in 2023.
```{r}
unique_words_2021 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
This creates a dataframe with the top-10 two word phrases for the press releases from 2021. The top phrase for this year was 'covid-19' which makes a lot of sense given the circumstances of that year. Human rights and pandemic were the next top words.
```{r}
releases %>%
  filter(date < '2022-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
This does the same thing as the code before but for the year 2022. We can see that 'project location' was the top two-word phrase for that year.The next phrases were 'amount included' and 'health care.'
...
**Task** Run the following code and describe the results.
**Answer**
This does the same thing as the code before but for the year 2023. We can see that 'human rights' was the top two-word phrase for that year.The next phrases were 'equal rights' and 'health care.'
...
**Answer**
This code calculates the sentiment of prevalent words in the text by creating a new data frames by each year and including a column that says if a word is either positive or negative. It sorts them by how many times that word appears in the text.
```{r}
