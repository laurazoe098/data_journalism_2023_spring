...
**Answer**
This loads a dataframe that shows Maryland zip codes and data that represents different aspects of it's geometry.
```{r}
...
**Answer**
It contains columns that we have used before such as zip codes or different characteristics like shape area. The only new thing that it has is the column with multipolygon in it showing that it has multiple data inside of it.
```{r}
View(md_zips)
```
...
**Answer**
The output is a map of Maryland divided by it's different jurisdictions or zip codes.
```{r}
md_zips %>%
  ggplot() +
  geom_sf() +
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The following code loads a library called tigris.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This has all the county boundaries in the United States from the year 2021.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This filters the previous code to only show Maryland counties using Maryland's code of 24.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code shows a map of Maryland divided by counties. The Chesapeake Bay is not outlined because it is part of certain Maryland counties and displayed like that.
```{r}
md_counties %>%
  ggplot() +
  geom_sf() +
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code displays a map with both county and zip code boundaries. Some county boundaries include the Chesapeake Bay while the zip codes do not include it.
```{r}
ggplot() +
...
**Task** Run the following code. Describe the output in the space below.
**Answer** 
This code created a data frame that shows the number of foreclosures for each Maryland zip code in the month of March of this year (2023).
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code creates a new dataframe that combined the Maryland zip code data with the data containing the number of foreclosures by Maryland zipcode.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code uses the data we just created and shows a map that uses lighter colors show the most notices in Maryland by zip code.
```{r}
ggplot() +
...
**Answer**
This code loads the library's we were using before as well as the tidy census so that we are able to use census data.
```{r}
library(tidyverse)
library(sf)
library(janitor)
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code creates a data frame that shows the population per Maryland county using the census. It also loads in a frame that shows the eviction notices in Maryland by zipcode.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This takes the same dataframe we created before and adds a new geometry column.
```{r}
md_county_population <- get_acs(geography = "county",
              variables = c(population = "B01001_001"),
              year = 2021,
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This loads both of the data frames we just created.
```{r}
md_county_population
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This changed the md_county_population dataframe selected only three columns from the previous dataframe and shows county, population and geometry. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code creates a new data frame that has combined the foreclosure data with the county population data combining them by the county column.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code calculates the number of forclosure notices per 1000 population in each county.
```{r}
md_pop_with_foreclosures <- md_county_population %>%
  left_join(md_county_notices, by='county') %>% 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code produces a map that shows the number of evictions per 1000 people in each Maryland county. It shades in lighter counties with higher rates according to the legend.
```{r}
ggplot() +
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code does a better job of distinguishing differences between rates by using a bigger palette of colors to represent the data.
```{r}
ggplot() +
  geom_sf(data=md_pop_with_foreclosures, aes(fill=rate)) +
  theme_minimal() +
...
**Answer**
This code made the colors more distinguishable than the last code so that it the difference between counties is more noticeable.
```{r}
ggplot() +
  geom_sf(data=md_pop_with_foreclosures, aes(fill=rate)) +
  theme_minimal() +
