---
title: Reverse Engineering Project By: Laura and Natalie
...
owner_totals <- sports_politics %>%
 ...
 owner_totals_added <- owner_totals%>%
  mutate( total = democrat+ n_a + bipartisan + republican + bipartisan_but_mostly_republican +   bipartisan_but_mostly_democratic + independent, pct_dem=democrat/total, pct_rep=republican/total)
...
owner_totals %>%
  group_by(league)%>%
 summarise(sum(democrat+ n_a + bipartisan + republican + bipartisan_but_mostly_republican +   bipartisan_but_mostly_democratic + independent))
#total sum = 3,162,455
...
#The total of these donations is 29,692,044. When divided by $47 million, the total donations, it is 63%.
...
sports_politics%>%
  ...
  filter(amount >= 100000)%>%
  summarise(sum(amount))
#The total of the Republican donation is 23,429,805. When divided by $47 million, the total donations, it is 79%. 
