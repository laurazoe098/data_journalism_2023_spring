...
The warning says "One or more parsing issues, call `problems()` on your data frame for details, e.g.: dat <- vroom(...)." The max needs to be more than 10.
...
It does now show a parsing error because the maximimum has been expanded to include all the rows.
...
**Answer**
1,094 zip codes do not have five characters. I think this is happening in the data because someone opened the data in excel causing the leading zeros in each zip code to be dropped.
...
Some records in 2010, 2011 and 2017 where a grant or loan is uncategorized. One of the categories is called "L" which is probably referring to a loan but we would need to verify.
...
**Answer**
I see that the smallest grant amount is $60, this is not a lot of money compared to the rest of the money that the government is giving out which is odd. This could be a potential outlier. Additionally, some of the amounts are even numbers while others have are not.
```{r}
md_grants_loans %>% 
  arrange(Amount)
```
...
It creates a new data sheet with column names that contain no spaces, they are replaced by underscores. All the capitalized words are now lowercase as well. It has basically cleaned up our data to be consistent throughout.
```{r}
# cleaning function
...
There are 50 duplicates possible.
...
The new dataframe now has 17,740 rows.
...
Now, all the zipcodes in the "zip5" column are 5 digits. This changed the zipcodes that were 9 digits to 5.
```{r}
# cleaning function
cleaned_md_grants_loans <- md_grants_loans %>%
  clean_names() %>% 
...
ANSWER: The zip code that has received the most amount of money from the Maryland Tourism Board is "21202." They received a total of $2,172,512.00	
