...
**Answer**
First, mutate adds a month column with the abbreviations of the months. Then the code groups by month and adds the total of calls by that month. It sorts the data from the most calls to the least. 
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**
I see a bar chart that visualizes the total amount of calls per month.
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
...
**Answer**
It uses the reorder function so that the bars are ordered by the total number of calls, lowest to highest. 
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
In this chart, the x and y axis have been switchd. So now the month is in the y axis and the total is in the y axis.
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The grey background is now gone.
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Answer**
It has a different color scheme. The background is now a light blue-grey. The fonts are now different too.
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
Now the bar chart has an appropriate title at the top. The x and y axis labels also are different and more accuretly represent the data.
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
Att the batltimore calls have been groups by the date it happened and added up.
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
I see a line chart that shows where calls spiked during the year and where there were less.
```{r}
baltco_911_calls_by_date %>%
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
Now we see the same line chart but it shows the date by one week instead of by month. The specific dates are also labeled.
```{r}
baltco_911_calls_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
Now the x axis labels are more legible since they are now at an angle.
```{r}
baltco_911_calls_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 
