...
```{r}
baltimore_county_od <- read_csv("tabula-Baltimore County; Carey, Samantha log OD.csv", col_names =   FALSE) %>% clean_names()
baltimore_county_od <- baltimore_county_od %>% mutate(x1=mdy(x1))%>%
...
A1. 
The date in 2022 that had the most overdose calls was July 14, 2022 with a total of 23 calls.Based on the number of rows that there are (366) I do not think that there are any days with no overdose calls.
...
A2. 
I found the days with the highest percentage of overdoses are Saturday, Sunday and Friday. These are all weekends and these seem to be a prime time for overdose activities.Monday, Wednesday and Thursday are all weekdays and have the lowest percentage of overdoses. 
```{r}
baltimore_county_od$weekday <- wday(baltimore_county_od$date,label=TRUE)

baltimore_county_od %>%
  group_by(weekday) %>%
  summarise(total_weekday = n()) %>%
  mutate(prt_calls = total_weekday/4112)%>%
  arrange(desc(total_weekday))
...
A3. 
The address that has the most calls is "4540 Silver Spring RD." The top three addresses are all in the Baltimore area which could indicate that the area is more prone to overdoses. I would say the address data is not that clean so these results might be a little off but it does provide a general overview as to what areas have more overdoses than others. I would say that the data containing the dates is more reliable since it has been cleaned.
```{r}
baltimore_county_od %>%
  group_by(address) %>%
  summarise(total_address = n()) %>%
  arrange(desc(total_address))
```

Q4. What's the best story idea or question you've seen as a result of the work you've done in this lab?

A4. 
The best story idea I can think of is probably examining what addresses and surrounding areas contain the most overdoes. This could also be really interesting as a data visualization. I also think the fact that most overdoses occur on the weekend would also be an interesting story, I would be curious to know the ages of the people who overdosed and see if that correlates with what day the incident occured.
