...
**Answer**
The dataframe that is outputted is of states and what percentage of voters voted for Trump in the 2016 elections, what percent of businesses received PPE and what amount of employees received that PPE.
```{r}
...
**Answer**
The plot that is outputted shows that there is a positive correlation between the percent of business that received PPE and the percent of voters that voted for Trump. So basically, the more red the state the more PPE businesses in that state received.
```{r}
...
**Task** Run the following code. Describe the plot. What do you see?
**Answer**
This plot shows that there is a slight positive correlation between PPE amount per employee and the percent of voters who voted for Trump. It is not as clear as the first plot however.
```{r}
...
**Answer**
This correlates the trump percentage to the other numeric values, the pct business received from PPE and the PPE amount per employee. This puts a number, or r-value to the graphs above.
```{r}
reveal_data %>%
...
**Answer**
This tests these two relationships to see how valid they are. The output shows that this relationship could have just happened by chance because the p-value is very low.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**
This tests the relationship between pct_trump and the ppe_amount_per_employee and shows that the r-value is .22 which is pretty weak and the p-value is 0.11 which is also showing that there is little correlation between these two values.
```{r}
...
**Answer**
The table shows the mean PPE amount per employee and mean PPE businesses receive between states who voted for Trump and states who voted for Hillary Clinton. 
```{fr}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer**
I see two means for both Trump and Clinton. I also see that the t-value is almost 1 and the p-value is .2295.
```{r}
...
**Task** Run the following code. Describe the output. What do you see?
**Answer**
I see that the t-value is about 3 and the p-value is .002 showing that this is finding is meaningful. 
```{r}
