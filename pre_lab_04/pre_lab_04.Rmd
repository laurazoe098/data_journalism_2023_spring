...
The data is not clean so some grantees are labeled similar names that are different by one letter. For example WASHINGTON CO COMMISSIONERS" is also labeled WASHINGTON CO "COMMISSIONE." I am assuming these are the same but they are spelled differently. Even though it is a minor difference, the data still reads it as two different organizations.
```{r}
...
md_grant_loans %>%
  ...
  filter(str_detect(Grantee, '^w|^W')) %>% #only show me the grantees that begin with a upper or lowercase w
  arrange(Grantee)
```
...
**Answer**
One change that the refinr package made was change "Washington County Museum of Fine Arts" to "WASHINGTON COUNTY MUSEUM OF FINE ARTS." This means that another entry was all capitalized so the package was standardizing all the grantee variations of that organization. Another change that the package made was "Charles County Arts Alliance Inc" which it changed to "CHARLES COUNTY ARTS ALLIANCE."
...
**Answer**
It capitalized all the grantee names. It also grouped by and counted these names by the cleaned up data from before. It also counted how many entry's each grantee had.
...
**Answer**
It made the "WASHINGTON CO COMM ACTION COUNCIL INC" from previously to all say "WASHINGTON CO COMM. ACTION COUNCIL INC." There are still some entries that have not been fully combined or cleaned up, however. For example "there are "WASHINGTON CO COMMISSIONE" and "WASHINGTON CO COMMISSIONERS" are still listed as two different grantees.
```{r}
cleaned_md_grant_loans <- md_grant_loans %>%
