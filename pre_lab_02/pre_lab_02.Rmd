...
umd_courses %%
  filter (seats >= 50) %%
  group_by(term) %%
  summarize(total_classes = n()) %%
  arrange(dsc(total_classes))
```
...
**Answer**
The OR operator gives us the number 37480.
...
**Answer:**
The change in the expenditure date is that it changed from being a "<chr>" to be a "<date>." This caused the format of the date to change. Now it starts with the year, then month, then day, and it is separated by dashes.
...
In this new month column, all of the dates are now lumped together by month. The column next to the month column is how many dates there are in that month.
...
**Answer**
This code is adding up all of the votes for each candidate and then dividing the voted for Moore by the total number of votes. Then it is creating a new column where the results will be displayed, showing the percentage of votes that went to Moore.
...
**Answer**
Now the code has multiplied the newest column by 100 to display a percentage instead of a decimal. It has also sorted the columns so that the counties with the highest percentage of votes for Moore are shown first and decrease going down.
...
**Answer**
Personally I think it is more interesting, only because I already had a good idea of what counties in Maryland are highly democratic. The counties who had the a lower percentage of votes for Moore are less familiar to me. 
...
**Answer**
One function that is similar to str_to_upper is the str_to_lower()function. It does the same thing expect it converts a character column into all lowercase. Similarly the str_to_title() function converts a character column so that the first letter of each word is capitalized.
...
**Answer**
I think that the mutate statement is is going through all the street addresses and detecting for the work "Maryland." If it detects this word then it puts MD in the state column. This is being done to multiple other states as well.
...
**Answer**
Yes, this answer makes sense to me. The most money came from Maryland which makes a lot of sense. The next two are Virginia and Washington which also makes a lot of since since they are neighboring territories.
...
1.What purpose did most of the expenditures come from?

2.What committee from the state of Maryland gave the most money?

3.What month were most of the expenditures recieved?
